...
import { AdminConfig } from '../vista-admin.config';
...
export class VistaAdminService {
  ...
  constructor( private http: HttpClient ) {
    this.resourceUrl = AdminConfig.httpResource.solicitudes;
  }
/*
...
*/
  get(user:string, password:string): Observable<any> {
    return this.http.get<ResultMessage>(`${this.resourceUrl}/${user}/${password}`, httpOptions).map(response=> response);
  }
/*
